...
for status in range(7, 10):  # adder to dic
    accepted = []  # this list need for deleting from cache
    for t in dic_fac:  # 't' - faculty name, sorting and compare(max) avg and special exam here >>>
        cache = sorted(cache, key=lambda x: (-max((float(x[ex_ind[t][0]]) + float(x[ex_ind[t][1]]))
                                                  / 2, float(x[6])), x[0], x[1]))
        for i in cache:  # player`s line
            if i[status] == t and len(dic_fac[t]) < n:  # slicing here >>>
                dic_fac[t].append([i[0], i[1], max((float(i[ex_ind[t][0]]) + float(i[ex_ind[t][1]]))
                                                   / 2, float(i[6]))])  # and compare avg and exam here <<<
                accepted.append(i)
